...
var
  Pixels: Single;
begin
  Pixels := Round(LogicPoints * Screen.ScreenScale);

  if (Pixels < 1) and AtLeastOnePixel then
    Pixels := 1.0;

  Result := Pixels / Screen.ScreenScale;
end;

function RoundToPixel(const Source: Single; const AThickness: Single = 1): Single; overload;
begin
  Result := Source;
  if SameValue(Round(Source * Screen.ScreenScale), Source * Screen.ScreenScale, EPSILON_SINGLE) then
